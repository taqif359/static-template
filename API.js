...
export class LocationPage extends Component {
  ...
  getRecyclingCenter = async e => {
    ...
    const api_call = await fetch(
      `https://cors-anywhere.herokuapp.com/http://api.earth911.com/earth911.searchLocations?api_key=${API_KEY}&latitude=${
        this.state.userLatitude
      }&longitude=${this.state.userLongitude}&max_distance=5`
    );
    ...
    if (response.result) {
      ...
      const api_call2 = await fetch(
        `https://cors-anywhere.herokuapp.com/http://api.earth911.com/earth911.getLocationDetails?api_key=${API_KEY}&location_id=${
